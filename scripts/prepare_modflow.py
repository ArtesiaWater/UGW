...
mod_areas_shp = r'..\data\modflow\waterareas.shp'
mask_shp = r'..\data\modflow\mask.shp'
w_lines_clipped_shp = r'..\data\modflow\waterlijnen_masked.shp'
...
wl_areas_gdf.insert(2,'Z',[stat['percentile_50'] * scale for stat in raster_stats])
...
mod_areas_gdf['src_id_wl'] = mod_areas_gdf.apply((lambda x: sjoin_lines(x)),axis=1)
...
drop_cols = [col for col in mod_areas_gdf.columns if not col in list(verd_gdf.columns)]
