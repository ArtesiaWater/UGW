...
def sjoin(row):
    ...
    if len(lines_gdf) == 0:
        ...
        src_id_wl = 'NL.{:.0f}.{}'.format(row['admin_id'],lines_gdf['src_id'].values[0])
    ...
    if len(areas_gdf) == 0:
        ...
        src_id_wla = 'NL.{:.0f}.{}'.format(row['admin_id'],areas_gdf['src_id'].values[0])
...
logging.info('indeces build in {} seconds'.format(round(time.time() - istart,1)))
...
def combine_wl(gdf):
    ...
    gdf['SUB_wla'] = 0
    clean_cols = [col for col in gdf.columns if not col in ['admin','admin_id','geometry','src','src_id']]
...
#        has_wp = False
        ...
        params = ['VP','FPB','FPO','ZP','ZFPB','ZFPO','WP','WFPB','WFPO']
        for param in params:
...
#                    has_zp = True
                    ...
                    verdict_gdf.loc[index,'ZP'] = '{}error:no ZP (estimated Z - 0.5);'.format(verdict_gdf.loc[index,'ZP'])  
                    row['ZP'] = row['Z50'] - 0.5
                    mark = 1
#            else:
#                has_zp = True
...
#                    has_wp = True
                    ...
                    verdict_gdf.loc[index,'WP'] = '{}error:no WP (estimated Z - 0.7);'.format(verdict_gdf.loc[index,'WP'])
                    row['WP'] = row['Z50'] - 0.7
                    mark = 1
#            else:
#                has_wp = True
            ...
            if not row['WP'] <= row['ZP']:
                verdict_gdf.loc[index,'WP'] = '{}doubtfull:WP>ZP;'.format(verdict_gdf.loc[index,'WP'])
                verdict_gdf.loc[index,'ZP'] = '{}doubtfull:ZP<WP;'.format(verdict_gdf.loc[index,'ZP'])
                mark -= 1
...
w_lines_df = pd.DataFrame(w_lines_df.drop(['admin','admin_id','src_id','src'],axis=1))
...
wl_areas_df = pd.DataFrame(wl_areas_df.drop(['admin','admin_id','src_id','src'],axis=1))
...
mod_areas_gdf[attrib] = mod_areas_gdf.apply((lambda x: merge_attrib(x,attribs,np.NaN)),axis=1)
...
def validate_wa(row):
    ...
    if row['BB'] == row['BB'] :
        if row['BB'] > row['geometry'].exterior.length/np.pi:
            BB_vdct = '{}error: BB unrealistic large (deleted);'.format(BB_vdct)
            row['BB'] = np.NaN
            mark -= 1
        elif row['BB'] < 0.5:
...
verdict_parameters = [col.replace('_vdct','') for col in ma_verdict_gdf.columns if '_vdct' in col]
