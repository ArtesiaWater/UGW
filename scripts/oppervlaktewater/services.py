...
class ArcREST:
    ...
    def get_features(self,layer,poly=None,object_filter='',object_id=None):
        ...
        if object_id == None:
            properties = requests.get(('{url}/{layer}/?f=pjson').format(url=self.url,layer=layer)).json()
            
            if "uniqueIdField" in list(properties.keys()):
                object_id = properties["uniqueIdField"]["name"]
            else:
                if "fields" in list(properties.keys()):
                    field = [field['name'] for field in properties["fields"] if field['name'].lower() == "objectid"]
                    if len(field) == 1:
                       object_id = field[0]
            if object_id == None:
                logging.error('Processing data from the following url failed: {url}/{layer}/?f=pjson'.format(url=self.url,layer=layer))
        ...
        try:
            if not object_filter == '': object_filter = ' and {}'.format(object_filter)
            url = ('{url}/{layer}/query?'
            ...
            if response.status_code == 200:
                ...
                if object_ids == None:
                    ...
                    for download in range(downloads):
                        ...
                        max_object = min(min_object + self.maxRecordCount-1,len(object_ids)-1)
                        url = ('{url}/{layer}/query?'
                        ...
                        if len(response.json()['features']) > 0:
                            if self.format == 'json':
                                logging.warning('reading ESRI-json format (GeoJSON is preferred)')
                                vsif = fiona.ogrext.buffer_to_virtual_file(bytes(response.content))
                                vsiz = vsif+'.json'
                                gdal.Rename(vsif, vsiz)
                                fc = fiona.Collection(vsiz)
                                gdf = gpd.GeoDataFrame.from_features([feature for feature in fc], crs=self.crs)
                            ...
                            logging.warning('no features returned for url: {}'.format(url))
                    ...
                    layer_name = [lay['name'] for lay in self.layers if lay['id'] == layer][0]
                    gdf['layer_name'] = layer_name
                    
                    return(gdf)
            
        except Exception as e:
            logging.error('Processing data from the following url failed: {} with error {}'.format(url,e))
...
class WFS:
    ...
    def __init__(self,url,timeout=300):
        ...
        self.capabilities = ET.ElementTree(ET.fromstring(response.text)).getroot()
        ...
        operations = self.capabilities.find('ows:OperationsMetadata'
                                           ,self.capabilities_ns).findall('ows:Operation',
                                                                         self.capabilities_ns)
        ...
        if 'application/json' in self.output_formats:
            ...
            gml_formats = [output_format for output_format in self.output_formats if 'text/xml' in output_format]
            if len(gml_formats) > 0:
               self.output_format = gml_formats[0]
        
        if self.output_format == None:
            logging.warning('no suitable output format available from {}'.format(self.output_formats))
        else:
    ...
    def get_features(self,layer,crs,poly=None):
        if layer in self.layers:
            ...
            if src_crs == None:
                src_crs = feature_type.find('wfs:DefaultCRS',self.capabilities_ns)
            ...
            gdf = wfs2gdf(url,src_crs,output_type=self.output_format,timeout=self.timeout)           
