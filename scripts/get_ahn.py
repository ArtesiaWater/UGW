...
def to_tif(mask_poly,tif_file,layer='ahn3_05m_dtm',cell_size=0.5,src_nodata=None,overviews=[5,25]):
    ...
    xmin, ymin, xmax, ymax = [item['bbox'] for item in bboxes if item['nativeSrs'] =='EPSG:28992'][0]
    wcs_poly = Polygon([(xmin,ymin),(xmin,ymax),(xmax,ymax),(xmax,ymin),(xmin,ymin)])
    if not wcs_poly.intersects(mask_poly):
        ...
        with rasterio.open(tif_file,'w',**profile) as dst:
            ...
            if not overviews == None:
                ...
                factors = [int(size/cell_size) for size in overviews if size > cell_size]
...
def io(file_in,tif_file,layer='ahn3_05m_dtm',cell_size=0.5,src_nodata=None,overviews=[5,25]):
    ...
    to_tif(mask_poly,tif_file,layer=layer,cell_size=cell_size,src_nodata=src_nodata,overviews=overviews)
