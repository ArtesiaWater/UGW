...
def get_centerline(geom, segmentize_maxlen=0.5, max_points=3000,
    ...
    if geom.geom_type == "Polygon":
        ...
        if len(end_nodes) < 2:
            ...
            raise Exception("Polygon has too few points")
        ...
        # for MultPolygon, only raise Exception if all subgeometries fail
        if sub_centerlines:
            return MultiLineString(sub_centerlines)
        else:
            raise Exception("all subgeometries failed")
...
def get_centerline_simple(polygon, simplify=10, maxnum=5):
    ...
    simplify : The parameter passed to the simplify method of shapely. Do not
        when set to None. Defult to 10.
    ...
    if not isinstance(polygon, Polygon):
        raise(Exception('Input has to be of type Polygon'))
    if simplify is not None:
        polygon = polygon.simplify(simplify)
    vor = Voronoi(polygon.exterior)
    graph = _graph_from_voronoi(vor, polygon)
    end_nodes = _get_end_nodes(graph)
    if len(end_nodes) < 2:
        raise Exception("Polygon has too few points")
