...
def plot_timeseries_result_vertex_grid(hdobj, gwf, model_ds, x, y, lay=0):
    ...
    # get model heads
    hds_idx = [hdobj.get_data(totim=tim, mflay=lay)[0][idx] for tim in hdobj.times]
...
def plot_map_cross_section_at_line_shp(shp_fname, model_ds, gwf, gdf_opp_water, panden_shp,
    ...
    fig.savefig(os.path.join(figdir,f'map_view_cross_section_{cross_section_name}.png'))
    ...
    ax.set_ylim(*ylim)
    t = ax.set_title(f"Cross-Section {cross_section_name} with Horizontal hydraulic conductivity")
    
    cb = plt.colorbar(csa, shrink=0.75)
    
    fig.savefig(os.path.join(figdir,f'cross_section_{cross_section_name}.png'))
...
def plot_map_cross_section_at_y(y, model_ds, gwf, gdf_opp_water, 
    ...
    linecollection = xsect.plot_grid()
    ax.set_ylim(*ylim)
    t = ax.set_title(f"y: {y} Cross-Section {cross_section_name} with Horizontal hydraulic conductivity")
    cb = plt.colorbar(csa, shrink=0.75)
...
def plot_map_cross_section_at_x(x, model_ds, gwf, gdf_opp_water, panden_shp,
    ...
    linecollection = xsect.plot_grid()
    ax.set_ylim(*ylim)
    t = ax.set_title(f"x: {x} Cross-Section {cross_section_name} with Horizontal hydraulic conductivity")
    cb = plt.colorbar(csa, shrink=0.75)
...
def plot_layer_stitches_y(y, model_ds, gwf, gdf_opp_water,
    ...
    t = ax.set_title(f"y: {y} Cross-Section {cross_section_name} with layer numbers")
    cb = plt.colorbar(csa, shrink=0.75)
...
def plot_layer_stitches_x(x, model_ds, gwf, gdf_opp_water,
    ...
    t = ax.set_title(f"x: {x} Cross-Section {cross_section_name} with layer numbers")
    cb = plt.colorbar(csa, shrink=0.75)
...
def compare_layer_models_cross_section(y, lay_mod1, lay_mod2, lay_mod1_bot_name='bottom',
    ...
    #y = 502000.
    row_lay_mod1 = np.where(lay_mod1.y == (lay_mod1.y.sel(y=y, method='nearest').data))[0]
    row_lay_mod2  = np.where(lay_mod2.y == (lay_mod2.y.sel(y=y, method='nearest').data))[0]


    fig = plt.figure(figsize=(10, 10))

    z1 = lay_mod1[lay_mod1_bot_name][:,row_lay_mod1]
    plt.plot(lay_mod1['top'][0,row_lay_mod1].x.data, lay_mod1['top'][0,row_lay_mod1][0], c="k")
    for zi in z1:
    ...
    for i in range(len(z1)-1):
        if i==layer_mod1:
            c = "lightblue"
            plt.fill_between(z1.x.data, z1[i][0], lay_mod1['top'][i,row_lay_mod1][0], alpha=0.5, color=c)
        # else:
    ...
    plt.plot(lay_mod2['top'][0,row_lay_mod2].x.data, lay_mod2['top'][0,row_lay_mod2][0], c="r")
    for zi in z2:
        plt.plot(zi.x.data, zi[0], c="r", ls="dashed")
    for i in range(len(z2)-1):
         if i==layer_mod2:
            c = "lightcoral"
            plt.fill_between(z2.x.data, z2[i][0], lay_mod2['top'][i,row_lay_mod2][0], alpha=0.5, color=c)
...
def plot_comparison_modellayers_topview(lay_mod1, lay_mod2, 
    ...
    im = ax.matshow(compare, cmap=cmap, norm=norm, extent=extent, aspect='auto')
