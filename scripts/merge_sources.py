...
def explode(gdf):
    ...
    outgdf = outgdf.append(MultiPolgyon2Polygon(gdf[gdf.geometry.type == "MultiPolygon"])
                           ,ignore_index=True)   
    for idx, row in gdf[gdf.geometry.type == "GeometryCollection"].iterrows():
        geoms = [geom for geom in row['geometry'] if geom.type in ['Polygon','MultiPolygon']]
        for geom in geoms:
...
def get_layer(src_layer,admins_gdf):
    ...
    if src_layer == 'water_areas':
        bgt_gdf = admins_gdf[admins_gdf['name'].isin([key for key, value in sources[src_layer].items() if value['source'] == 'bgt'])]
        bgt_names = list(bgt_gdf['name'].values)
        if len(bgt_gdf) > 0:
            logging.info('bgt for {}'.format(", ".join(list(bgt_gdf['name'].values))))
            bgt_gdf['dissolve'] = 0
            ...
            poly = Polygon([[xmin,ymax],[xmax,ymax],[xmax,ymin],[xmin,ymin],[xmin,ymax]])
            
            bronhouders = [value['bronhouder'] for key, value in sources[src_layer].items() if key in bgt_gdf['name'].values]
            ...
            valid_bronhouders = [bronhouder for bronhouder in np.unique(gdf['bronhouder'].values) if (bronhouder in bronhouders) | (not bronhouder[0] == 'W')]
            ...
            gdf[['admin_id','admin']] = gdf.apply(bgt_admin_conversion, axis=1, result_type="expand")
            
            drop_cols = [col for col in list(gdf.columns) if not col in ['gml_id','admin_id','admin','geometry']]
            ...
            admins_gdf = admins_gdf[admins_gdf['name'].isin([key for key, value in sources[src_layer].items() if value['source'] != 'bgt'])]
        ...
        if admin in list(sources[src_layer].keys()):
            ...
            if src['source'] in list(sources['services'].keys()):
                ...
                for layer in layers:
                    if src_url['type'] == 'wfs':
                        ...
                        if layer in wfs.layers:
                            gdf = wfs.get_features(layer,crs=sources['default_crs'],poly=poly)
                        else:
                            logging.error('{} not a layer in {}'.format(layer,src['source']))
                        ...
                        gdf = rest.get_features(layer,poly,object_filter=object_filter,object_id=object_id)
                    
                    elif src_url['type'] == 'gdb':
                        gdf = gpd.read_file(url, driver='FileGDB', layer=layer, bbox=poly)
                ...
                if len(gdf_tmp_list) == 1:
                    ...
                    gdf = gpd.GeoDataFrame(pd.concat(gdf_tmp_list, ignore_index=True))
            ...
            if len(gdf) > 0:
                if 'attributes' in list(src.keys()):
                    ...
                    merge_attributes = {key:item for key, item in attributes.items() if type(item) == list}
                    ...
                    if len(merge_attributes) > 0:
                        ...
                        attributes = {key:item for key, item in attributes.items() if not type(item) == list}
                ...
                if (not len(gdf_invalid)) == 0 & all(geom_type in ['Polygon','MultiPolygon'] for geom_type in np.unique(gdf['geometry'].type)):
                    logging.warning('trying to fix geometry errors by 0m buffer')
                ...
                logging.warning('data-source does not contain features for this project-extent')                
        
        else:
            logging.warning('source for {} not specified in {}'.format(admin,src_layer))
    ...
    if len(gdf_invalid_list) > 0:
        gdf_invalid = gpd.GeoDataFrame(pd.concat(gdf_invalid_list, ignore_index=True))
        gdf_invalid.crs = sources['default_crs']
        gdf_invalid.fillna(value=np.NaN, inplace=True)
    else: gdf_invalid = gpd.GeoDataFrame()
...
def schema(gdf):
    ...
    schema = {'geometry':min(list(np.unique(gdf['geometry'].type.values)),key=len)}
...
for src_layer in src_layers:
    ...
    if 'GeometryCollection' in np.unique(gdf.geometry.type):
        gdf = explode(gdf)
    if not gdf.empty:
        gdf.to_file(input_dir.joinpath('{}.shp'.format(src_layer)),schema=schema(gdf))
    if not gdf_invalid.empty:
        gdf_invalid.to_file(input_dir.joinpath('{}_invalid.shp'.format(src_layer)),schema=schema(gdf))
